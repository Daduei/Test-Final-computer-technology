...
export default function WikiDashboard({ user, onLogout }) {
  ...
  // toolbar state: reflect current selection's font and size
  ...
  const applyFormat = (cmd, value) => {
    try {
      // Support a special custom command for precise pixel font sizes
      if (cmd === 'setFontSizePx') {
        // Use execCommand fontSize to wrap selection, then replace resulting
        // <font size="X"> tags inside the editable area with <span style="font-size:NNpx">.
        const px = parseInt(value, 10) || 14
        // Choose the largest font size token (7) so selection is wrapped.
        document.execCommand('fontSize', false, '7')
        if (editorRef.current) {
          const cur = editorRef.current
          // Replace any <font> elements generated by execCommand
          const fonts = cur.querySelectorAll('font')
          fonts.forEach((f) => {
  ...
  // update toolbar (font/size) based on caret/selection inside editor
  useEffect(() => {
    const updateFromSelection = () => {
      try {
        ...
        // climb up to an element inside editor
        while (node && node !== editorRef.current && node.nodeType === 1) {
          // stop at the first element node (closest ancestor)
          break
        }
        const el = node && node !== editorRef.current ? node : editorRef.current
        const cs = window.getComputedStyle(el)
        const fontFamily = (cs.fontFamily || '').split(',')[0].replace(/['"]/g, '')
        const fontSizePx = parseInt(cs.fontSize, 10) || selectedSize
        setSelectedFont(fontFamily || '')
        setSelectedSize(fontSizePx)
        // compute caret rect to position floating label
        try {
          if (sel.rangeCount > 0) {
            const range = sel.getRangeAt(0).cloneRange()
            let rect = range.getBoundingClientRect()
            if (!rect || (rect.width === 0 && rect.height === 0)) {
              // try to expand range to get a better rect
              range.setStart(range.startContainer, Math.max(0, range.startOffset - 1))
              rect = range.getBoundingClientRect()
            }
            if (rect) {
              const left = rect.left + window.scrollX
              const top = rect.top + window.scrollY - 36 // place above caret
              setFloatingPos({ left, top })
              setFloatingVisible(true)
              if (floatingTimeoutRef.current) clearTimeout(floatingTimeoutRef.current)
              floatingTimeoutRef.current = setTimeout(() => setFloatingVisible(false), 1600)
            }
          }
        } catch (err) {
          ...
          <>
            ...
            {normalizedSelected ? (
              <section className="wiki-card wiki-edit-section">
                  {/* floating font label (absolute in page) */}
                  {viewMode === 'edit' && floatingVisible && (
                    <div
                      className="rt-float"
                      style={{ left: floatingPos.left, top: floatingPos.top }}
                    >
                      {selectedFont || 'Font'}
                    </div>
                  )}
